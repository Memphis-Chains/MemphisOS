...
function parseExportResult(commandResult: CommandResult): ExportCommandOutput {
  ...
  try {
    return parseJsonObject(commandResult.stdout, 'ops:export-incident-bundle') as ExportCommandOutput;
  } catch (error) {
    return {
      ok: false,
      error:
...
function main(): never {
  ...
  const expectedKeyId = stringOrNull(parseArg('--expected-key-id')) ?? stringOrNull(exportParsed.signingKeyId);
  ...
  if (verifyRun.status !== 0 || !verifyParsed.ok) {
    const verifyErrors =
      Array.isArray(verifyParsed.errors) && verifyParsed.errors.length > 0 ? verifyParsed.errors : [];
    return emitSummary(
      buildSummary({
        ok: false,
        stage: 'verify',
