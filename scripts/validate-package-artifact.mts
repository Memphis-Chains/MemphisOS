...
function resolveArtifactPath(existingArtifactPath: string | null, tempDirs: string[]): string {
  ...
  if (packResult.status !== 0) {
    throw new Error(firstLine(packResult.stderr) ?? firstLine(packResult.stdout) ?? 'npm pack failed');
  }
...
function runCliProbe(packageDir: string, extractDir: string): CliProbeSummary {
  ...
  if (!summary.ok) {
    throw new Error(summary.stderrFirstLine ?? summary.stdoutFirstLine ?? 'packaged CLI probe failed');
  }
  if (summary.stdoutFirstLine !== '# bash completion for memphis') {
    throw new Error(`unexpected packaged CLI probe output: ${summary.stdoutFirstLine ?? '<empty>'}`);
  }
...
try {
  ...
  if (extractResult.status !== 0) {
    throw new Error(firstLine(extractResult.stderr) ?? firstLine(extractResult.stdout) ?? 'tar extraction failed');
  }
  ...
  print(summary, options.json);
  process.exit(summary.error?.startsWith('Unknown option:') || summary.error?.includes('requires a value') ? 2 : 1);
} finally {
