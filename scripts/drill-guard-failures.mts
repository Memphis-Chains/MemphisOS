...
async function runTrustRootStrictDrill(tempDir: string): Promise<DrillResult> {
  ...
  const ok = exitCode === EXIT_CODES.ERR_TRUST_ROOT && trust?.enabled === true && trust.valid === false;
  return {
...
async function runRevocationStaleDrill(): Promise<DrillResult> {
  ...
  const ok = revocation?.enabled === true && revocation.stale === true && (revocation.ageMs ?? 0) > 30_000;
  return {
