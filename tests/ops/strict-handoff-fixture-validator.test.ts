...
describe('strict-handoff fixture validation script', () => {
  ...
  it('emits machine-readable summary with stable key and check contracts in --json mode', () => {
    const result = spawnSync('npm', ['run', '-s', 'ops:validate-strict-handoff-fixtures', '--', '--json'], {
      cwd: repoRoot,
      encoding: 'utf8',
      timeout: 60_000,
      env: process.env,
    ...
    expect(Object.keys(parsed).sort()).toEqual([...validatorOutputContract.summaryTopLevelKeys].sort());
