...
describe('strict-handoff json schemas', () => {
  it('summary schema is present and aligned with output-contract keys', () => {
    expect(outputContract.summaryJsonSchemaPath).toBe('tests/fixtures/strict-handoff/summary.schema.json');
    ...
    expect(Object.keys(summaryProperties).sort()).toEqual([...outputContract.summaryTopLevelKeys].sort());
    ...
    expect(asStringArray(profiles.required).sort()).toEqual([...outputContract.summaryProfileKeys].sort());
    ...
    expect(asStringArray(artifacts.required).sort()).toEqual([...outputContract.summaryArtifactKeys].sort());
    ...
    expect(asStringArray(checks.required).sort()).toEqual([...outputContract.summaryCheckKeys].sort());
    expect(Object.keys(asRecord(checks.properties)).sort()).toEqual([...outputContract.summaryCheckKeys].sort());
  });
  ...
  it('completion-hints schema is present and aligned with output-contract keys', () => {
    ...
    const completionSchemaPath = path.resolve(repoRoot, outputContract.completionHintsJsonSchemaPath);
    ...
    expect(completionRequired.sort()).toEqual([...outputContract.completionHintTopLevelKeys].sort());
