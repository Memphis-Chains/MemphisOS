...
describe('managed app manifests', () => {
  ...
  vault_decrypt: (entryJson) => {
      ...
      JSON.stringify(
        {
          ...
          actions: {
            install: {
              ...
              steps: ['test "$(cat "$APP_STATE_DIR/token.txt")" = "secret-file-demo" && printf file-ready'],
              vaultFiles: {
    ...
    expect(plan.requirements.find((item) => item.id === `secret-file:${secretFilePath}`)?.ok).toBe(true);
