...
function writeWorkspaceContext(root: string, force: boolean, statuses: WorkspaceStatus[]): WorkspaceContext {
  const pathValue = workspaceContextPath(root);
  ...
  statuses.push({
    ...
    detail: existed ? 'workspace context rewritten from default template' : 'workspace context created',
  });
  return context;
}
...
function renderManagedContextFile(targetFile: 'AGENTS.md' | 'CLAUDE.md', context: WorkspaceContext): string {
  const title = targetFile === 'AGENTS.md' ? '# Workspace Agent Guide' : '# Claude Workspace Context';
  const intro =
...
function ensureWorkspaceDirectories(root: string, context: WorkspaceContext, statuses: WorkspaceStatus[]): void {
...
export function initializeWorkspace(pathValue?: string, options: { force?: boolean } = {}): WorkspaceMutationResult {
...
export function syncWorkspaceContext(pathValue?: string, options: { force?: boolean } = {}): WorkspaceMutationResult {
  const root = resolveWorkspaceRoot(pathValue);
  const statuses: WorkspaceStatus[] = [];
