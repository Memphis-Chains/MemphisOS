...
import type { ManagedAppExecutionResult, ManagedAppManifestRef, ManagedAppResolvedPaths } from './manifest.js';
...
export function loadManagedAppRegistry(rawEnv: NodeJS.ProcessEnv = process.env): ManagedAppRegistry {
...
function nextStateForAction(action: string, previous?: ManagedAppRegistryRecord): ManagedAppInstallState {
  switch (action) {
    case 'install':
      return 'installed';
    case 'start':
...
export function recordManagedAppExecution(
  ...
  registry.apps = [record, ...registry.apps.filter((app) => app.id !== record.id)].sort((left, right) =>
