...
export async function runVaultSetupInteractive(
  ...
  try {
    ...
    vaultInit(
      { passphrase: passphrase.trim(), recovery_question: question.trim(), recovery_answer: answer.trim() },
      envWithPepper,
    );
    return {
...
export async function runWizardInteractive(
  ...
  try {
    ...
    // 5. Vault setup
    const { result: vault, passphrase, question, answer } = await runVaultSetupInteractive(rl, secrets.vaultPepper);

    // 6. Print credential sheet
    process.stdout.write(
      formatCredentialSheet(secrets, passphrase, question, answer),
    );
