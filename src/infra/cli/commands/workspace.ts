...
function printWorkspaceHelp(json: boolean): void {
  print(
    {
      usage: 'memphis workspace init [path] [--force] [--json] | memphis context sync [path] [--force] [--json]',
      notes:
...
function printWorkspaceResultHuman(action: string, result: ReturnType<typeof initializeWorkspace>): void {
...
export async function handleWorkspaceCommand(context: CliContext): Promise<boolean> {
  ...
  if ((command === 'workspace' && subcommand === 'sync') || (command === 'context' && subcommand === 'sync')) {
