...
function printPlanHuman(plan: ReturnType<typeof planManagedAppAction> | ReturnType<typeof executeManagedAppAction>): void {
...
export async function handleAppsCommand(context: CliContext): Promise<boolean> {
  ...
  if (subcommand === 'run') {
    ...
    const installedRecord = context.args.apply ? recordManagedAppExecution(ref, result, process.env) : undefined;
    if (json) {
  ...
  if (['install', 'start', 'stop', 'restart', 'status', 'doctor', 'dashboard'].includes(subcommand)) {
    const ref = manifestRefFromContext(context);
