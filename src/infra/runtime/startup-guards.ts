...
export function evaluateTrustRootStartup(
  ...
  try {
    ...
    const roots = Array.isArray(parsed.rootIds)
      ? parsed.rootIds.filter((item): item is string => typeof item === 'string' && item.trim().length > 0)
      : [];
...
export function evaluateRevocationCacheStartup(
  ...
  const maxStaleMs = Math.max(1, parseIntSafe(rawEnv.MEMPHIS_REVOCATION_CACHE_MAX_STALE_MS) ?? 30_000);
  if (!enabled) {
    return {
      enabled: false,
      stale: false,
